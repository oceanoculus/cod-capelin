...
packages <- c("rworldmap", "sp", "rgdal", "robis", "raster", "dplyr", "rworldmap", "plyr", "weathermetrics", "ncdf4")
source("../src/ipak.R")
ipak(packages)
```
...
cods_sp <- SpatialPointsDataFrame(coords = xy, data = cods, proj4string = CRS("+proj=aea +lat_1=50 +lat_2=70 +lat_0=40 +lon_0=-60 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs")) # The CRS is used here is for the albers equal area projection.
```
...
for (i in 1:no_netcdf) {  
  ...
  temp_brick <- projectRaster(temp_brick, aea) 
    for (j in 1:nrow(cods_sp)) {  
      de <- cods_sp$depthlayerno[[j]]  # a variable for the observation depth layer
      yr <- (cods_sp$year[j])  # a variable for the observation year
      mth <- (cods_sp$month[j])  # a variable for the observation month
          if (brkyr == yr & brkmth == mth & brkvar == "temp.nc"){
              cods_sp$temp_surface[j] <- extract(x=temp_brick[[1]], y = cods_sp[j, ]) 
              if (is.na(de)){
                cods_sp$temp_depth[j] <- NA
              } else  
                cods_sp$temp_depth[j] <- extract(x=temp_brick[[de]], y = cods_sp[j, ])
          } else if (brkyr == yr & brkmth == mth & brkvar == "salinity.nc") {
              cods_sp$salinity_surface[j] <- extract(x=temp_brick[[1]], y = cods_sp[j, ]) 
              if (is.na(de)){
                cods_sp$salinity_depth[j] <- NA
              } else  
                cods_sp$salinity_depth[j] <- extract(x=temp_brick[[de]], y = cods_sp[j, ]) 
          } else if (brkyr == yr & brkmth == mth & brkvar == "chl.nc") {
              cods_sp$chl_surface[j] <- extract(x=temp_brick[[1]], y = cods_sp[j, ]) 
              if (is.na(de)){
                cods_sp$chl_depth[j] <- NA
              } else  
                cods_sp$chl_depth[j] <- extract(x=temp_brick[[de]], y = cods_sp[j, ]) 
          } else if (brkyr == yr & brkmth == mth & brkvar == "o2.nc") {
              cods_sp$o2_surface[j] <- extract(x=temp_brick[[1]], y = cods_sp[j, ]) 
              if (is.na(de)){
                cods_sp$o2_depth[j] <- NA
              } else  
                cods_sp$o2_depth[j] <- extract(x=temp_brick[[de]], y = cods_sp[j, ]) 
          } else if (brkyr == yr & brkmth == mth & brkvar == "mlp.nc") {
              cods_sp$mlp_surface[j] <- extract(x=temp_brick[[1]], y = cods_sp[j, ])
          } else if (brkyr == yr & brkmth == mth & brkvar == "ssh.nc") {
              cods_sp$ssh_surface[j] <- extract(x=temp_brick[[1]], y = cods_sp[j, ]) 
            
          }
     
    }
}
cods <- as.data.frame(cods_sp)
write.csv(cods,"../output/bio/presence_points_without_envdata_relooped.csv", row.names = FALSE)
head(cods_sp$chl_depth)
```
